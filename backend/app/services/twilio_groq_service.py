...
class TwilioGroqService:
    ...
    def generate_response(self, call_sid: str, user_speech: str) -> str:
        ...
        if not client:
            logger.error("Groq client is not initialized or GROQ_API_KEY is missing")
        ...
        try:
            ...
            logger.error(f"Error calling Groq API for call_sid {call_sid}: {e}")
    ...
    def clear_session(self, call_sid: str) -> None:
        ...
        if call_sid in self.sessions:
            ...
            logger.info(f"Cleared session state for finished call_sid: {call_sid}")
